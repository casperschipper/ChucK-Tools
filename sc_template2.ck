class Main extends st {
    ...
    fun Stream four() {
        return line( seq([100,200,50,25,300]).set(cs.rv(0,2)), rv(1,10));
    }
    
    fun Stream two() {
        return t( boundedMupWalk(300,1200,ch([2.0/3.0,3.0/2.0])), ch([1,2,4,8,16]) );
    }
    ...
    fun Stream durlin(float ti) {
        return t( ch([0.4*ti,0.2*ti,0.4*ti,1.5*ti]) , ch([1,2,4,8,16]) );
    }
